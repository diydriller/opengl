...
void Viewer::Motion(int x, int y)
{
	...
	if (m_Mouse_Event[0])
	{
		...
		else if (interationMode)
		{
			...
			for (int i = 0; i < S_Simulator.cloth->nodes.size(); i++)
			{
				S_Simulator.cloth->nodes[i]->force.x += (float) 1.0f * (diffx * cosf(m_Rotate[0]) + diffx * sinf(m_Rotate[0]));
				S_Simulator.cloth->nodes[i]->force.z += (float) 0.5f * (diffx * cosf(m_Rotate[0]) - diffx * sinf(m_Rotate[0]));
				S_Simulator.cloth->nodes[i]->force.z += (float) 0.5f * (diffy * cosf(m_Rotate[1]) + diffy * sinf(m_Rotate[1]));
				S_Simulator.cloth->nodes[i]->force.y += (float) 1.0f * (diffy * cosf(m_Rotate[1]) - diffy * sinf(m_Rotate[1]));
			}
